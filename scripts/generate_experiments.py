...
def discretize_BNs(dist, exp):
    ...
    with open(output_file.format(dist, exp, exp_id), 'a', encoding='utf-8') as outfile:
        ...
        for meth, _bin in product(methods, bins):
            if meth!='MDLP':
                outfile.write(command.format(dist+exp, meth, _bin, bn_format, exp_id))
        # Add MDLP command
        if dist=='lg':
            outfile.write(command.format(dist+exp+" MDLP",2,"--target_variable E", bn_format, exp_id)) 
        if dist=='nm':
            outfile.write(command.format(dist+exp+" MDLP",2,"--target_variable Y", bn_format, exp_id)) 
        if dist=='tb':
            outfile.write(command.format(dist+exp+" MDLP",2,"--target_variable B", bn_format, exp_id)) 
        outfile.write("\n")
...
def main():
    ...
    with open(output_file.format(distribution, experiment, exp_id), 'w', encoding='utf-8') as outfile:
        print(f"Writing experiments script to {outfile.name}")
